...
/**
 * @brief Выполняет бинарный поиск ближайшего числа в отсортированном массиве.
 * 
 * Для каждого числа x находит индекс ближайшего числа в отсортированном массиве data.
 * Если таких чисел несколько, выводит индекс большего из них.
 * 
 * @param data Массив чисел, по которому будет выполняться бинарный поиск (массив отсортирован).
 * @param x Число, для которого нужно найти ближайшее значение в массиве.
 */
void myBinSearch(std::span<const int> data, int x) {
    ...
    // Выводим индекс ближайшего элемента
    std::cout << R << " ";
}
...
int main() {
    ...
    for (auto t : queries)
        myBinSearch(std::span<const int>(vec), t);  // Передаем отсортированный массив в функцию бинарного поиска
